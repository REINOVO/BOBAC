...
#include "bobac2_msgs/KitControl.h"
#define READ_SPEED 0
...
bool kit_callback(bobac2_msgs::KitControl::Request &req, bobac2_msgs::KitControl::Response &res){
    int ret;
    switch(req.kit_num){
        case 1:{
            if(req.kit_switch){
              
              ret = modbus_write_register(mb,12,55);
              if(ret == 1){
                res.kit_status = true;
              }
              else return false;
              ...
              ret = modbus_write_register(mb,12,80);
              if(ret == 1){
                res.kit_status = false;
              }
              else  return false;
            }
        }
        case 2:{
            if(req.kit_switch){
              ...
              ret = modbus_write_register(mb,13,48);
              if(ret == 1){
                res.kit_status = true;
              }
              else return false;
              ...
              ret = modbus_write_register(mb,13,85);
              if(ret == 1){
                res.kit_status = false;
              }
              else  return false;
...
void twistCallback(const bobac2_msgs::car_cmd & mycmd)
{
    ...
    int res = modbus_write_registers(mb,0,12,send_data);
...
int main(int argc, char **argv)
{
    ...
    ros::NodeHandle nh;
    car_data_pub = nh.advertise<bobac2_msgs::car_data>("car_data", 10);
    ros::Subscriber car_control_sub=nh.subscribe("car_cmd",10,twistCallback);
    ros::ServiceServer kit_server = nh.advertiseService("medical_kit", kit_callback);
