...
Control::Control()
{
    ...
    login();
    upload_userwords();
    control_service = m_handle.advertiseService("control",&Control::control_deal,this);
...
string Control::run_iat(const char* audio_file)
{
    string iat_result;
...
bool Control::control_deal(voice_msgs::control::Request &req,voice_msgs::control::Response &res)
{
    string iat_ret;
    iat_ret = run_iat(req.audio_file.data());	//识别音频中的语音，返回识别的文本
    if(iat_ret.find("前进") != string::npos){//如果语音有"前进"命令词
        res.control_order = "前进";//控制指令设为"前进"
    }
    else if(iat_ret.find("后退") != string::npos){//如果语音有"后退"命令词
        res.control_order = "后退";//控制指令设为"后退"
    }
    else if(iat_ret.find("左转") != string::npos){//同上
        res.control_order = "左转";
    }
    else if(iat_ret.find("右转") != string::npos){
        res.control_order = "右转";
    }
    else if(iat_ret.find("切换模式") != string::npos){//为后期多种模式切换提供命令词
        res.control_order = "切换模式";
    }
    else if(iat_ret.find("休息") != string::npos || iat_ret.find("睡觉") != string::npos){//让机器人不动
        res.control_order = "休眠";
    }
    else{
        res.control_order = "命令未识别";
    }
