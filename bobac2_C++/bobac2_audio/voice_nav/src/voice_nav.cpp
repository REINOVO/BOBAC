...
bool VoiceNav::voice_nav_deal(voice_msgs::voice_nav::Request &req,voice_msgs::voice_nav::Response &res)
{
    string iat_ret;
    iat_ret = run_iat(req.audio_file.data());		
    if(iat_ret.find("切换模式") != string::npos){
        res.nav_goal = "切换模式";
        goto flag;
    }
    else if(iat_ret.find("睡觉") != string::npos || iat_ret.find("休息") != string::npos){
        res.nav_goal = "休眠";
        goto flag;
    }
    else if(iat_ret.find("取消导航") != string::npos){
        res.nav_goal = "取消导航";
        goto flag;
    }
    else if(iat_ret.find("一圈") != string::npos){
        res.nav_goal = "导游";
        goto flag;
    }
    ...
    for(int i=0;i<position_name.size();i++){
        if(iat_ret.find(position_name[i]) != string::npos){
            res.nav_goal = position_name[i];
            cout<<position_name[i]<<endl;
            break;
        }
        else{
            res.nav_goal = "地点未识别";
