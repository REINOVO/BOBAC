...
Awake::Awake()
{
    ros::param::get("~awake_word",awake_word);
    ros::param::get("~userwords_file",userwords_file);
    login();
    upload_userwords();
    awake_service = m_handle.advertiseService("awake",&Awake::awake_deal,this);
...
string Awake::run_iat(const char* audio_file)
{
    string iat_result;
...
bool Awake::awake_deal(voice_msgs::awake::Request &req,voice_msgs::awake::Response &res)
{
    string iat_ret;
    iat_ret = run_iat(req.audio_file.data());		
    if(iat_ret.find(awake_word) != string::npos){
        res.awake_flag = 1;
    }
    else{
        res.awake_flag = 0;
